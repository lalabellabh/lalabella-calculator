  ...
  class NovaAssistant {
    ...
    toggleListening(){
      ...
      if(!this.speech.supported){
        alert('Voice recognition is not supported on this browser. Please use the buttons/manual entry instead.');
        return;
      }
    ...
    startSession(items, branch){
      ...
      if(first){
        ...
        this.respond('There are no items to count in this category.');
      }
    }
    ...
    async finishSession(){
      ...
      if(counted.length === 0){
        this.respond('No counts recorded. Session ended without saving.');
        this.endSessionUI();
        return;
      }
    ...
    async confirmSave(){
      this.awaitingSaveConfirm = false;
      this.respond('Who did the count? Say your name, or type it below.');
      ...
      try{
        ...
        if(res.error){
          this.respond('Error while saving: ' + res.error);
        } else {
          this.respond('Inventory saved successfully.');
          ITEMS = [];
        }
      }catch(e){
        this.respond('Not saved — please check your connection.');
      }
      this.endSessionUI();
    }
    ...
    cancelSave(){
      this.awaitingSaveConfirm = false;
      this.respond('Okay, not saved. Session ended.');
      this.endSessionUI();
    }
    ...
    handleTranscript(rawText){
      ...
      if(/^stop( counting)?$/i.test(lower)){
        this.speech.stop();
        this.respond('Paused. Say "resume" when you are ready.');
        return;
      }
      if(/^resume( counting)?$/i.test(lower)){
        const item = this.session.getCurrentItem();
        this.respond(item ? `Resuming. Next item: ${item.name}.` : 'No items remaining.');
        return;
      }
      ...
      if(/^repeat$/i.test(lower)){
        const item = this.session.getCurrentItem();
        this.respond(item ? `Current item: ${item.name}.` : 'No more items.');
        return;
      }
      ...
      if(/^(go back|back)$/i.test(lower)){
        ...
        this.updateDisplay();
        const item = this.session.getCurrentItem();
        this.respond(item ? `Back to ${item.name}. What's the quantity?` : 'We are already at the beginning.');
        return;
      }
      ...
      if(correctionMatch){
        ...
        if(ok){
          ...
          this.respond('No recorded items to correct yet.');
        }
        return;
      }
      ...
      if(/^next$/i.test(lower)){
        const item = this.session.getCurrentItem();
        this.respond(item ? `Next item: ${item.name}.` : 'No more items.');
        return;
      }
    ...
    parseItemAndQuantity(text){
      ...
      if(!currentItem){
        this.respond('No more items to count.');
        return;
      }
  ...
  document.getElementById('novaStartBtn').addEventListener('click', async ()=>{
    ...
    if(!sessionItems.length){
      alert('No items found for this branch/category.');
      return;
    }
    ...
    if(!novaAssistant){
      if(!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)){
        ...
        document.getElementById('novaUnsupportedMsg').textContent =
          'This browser does not support voice recognition. You can still type commands/quantities using the buttons below (Repeat/Skip/Back/Stop), but there is no mic input.';
      }
