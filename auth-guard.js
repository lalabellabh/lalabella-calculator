/**
 * LALABELLA AUTH GUARD — FAST SESSION + AUTH READY
 *
 * Loads the remembered session immediately, injects the session token into
 * application Google Apps Script requests, and prevents the first data/API
 * request from racing the initial authentication verification.
 */
(function () {
  ...
  function setGlobals(token, userRaw) {
    ...
    try { window.LALABELLA_USER = userRaw ? JSON.parse(userRaw) : null; }
  ...
  function clearSession() {
    try {
      ['lalabellaToken','lalabellaUser',VERIFY_AT_KEY,VERIFY_TOKEN_KEY].forEach(k => sessionStorage.removeItem(k));
    } catch (e) {}
    try {
      ['lalabellaToken','lalabellaUser',VERIFY_AT_KEY,VERIFY_TOKEN_KEY].forEach(k => localStorage.removeItem(k));
    } catch (e) {}
  }
  ...
  // No remembered session = real unauthenticated visit.
  if (!token) {
    setGlobals('', '');
    goToLogin();
    return;
  ...
  // Restore immediately. This keeps page navigation fast.
  ...
  async function verifyToken() {
    ...
    try {
      const url = AUTH_API_URL + '?action=verifyToken&token=' + encodeURIComponent(token) +
        '&callerSecret=' + encodeURIComponent(AUTH_CALLER_SECRET);
      const response = await originalFetch(url, { method:'GET', cache:'no-store', signal: controller ? controller.signal : undefined });
      ...
      // Unknown/malformed/temporary responses do not destroy a remembered session.
      return true;
    } catch (e) {
      return true;
    } finally {
  ...
  // Keep the UI instant, but gate application API requests behind the
  // initial auth check so pages never race into an Unauthorized response.
  let authReadyResolve;
  window.LALABELLA_AUTH_READY = new Promise(resolve => { authReadyResolve = resolve; });
  ...
  function prepareBackendRequest(input, init, currentToken) {
    ...
    if (isBackend && !isAuth && !/[?&]token=/.test(url)) {
      ...
      if (typeof input === 'string') nextInput = newUrl;
        ...
        try { nextInput = new Request(newUrl, input); }
        catch (e) { nextInput = input; }
      }
    }

    if (isBackend && !isAuth && nextInit && nextInit.body && typeof nextInit.body === 'string' && !/[?&]token=/.test(nextInit.body)) {
      ...
      nextInit = Object.assign({}, nextInit, { body });
  ...
  // Initial verification. Cached sessions resolve immediately; otherwise
  // API calls wait for this one verification (max 8 seconds).
  ...
  // Re-check after the tab has been hidden for more than one minute.
  let hiddenAt = 0;
