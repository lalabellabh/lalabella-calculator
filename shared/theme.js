 ...
 * - Pages already designed dark (KEEP_AS_IS, or detected) are left alone.
 * - Parts that must show TRUE colours (card/label previews, colour swatches)
 *   are declared per page in <head>:
 *     <meta name="lb-theme-keep" content="#card, .card, .plain-color-swatch">
 *   CSS rules whose selectors only target those, and anything inside them,
 *   are never converted.
...
(function () {
  ...
  function walkRules(rules) {
    for (var i = 0; i < rules.length; i++) {
      var r = rules[i];
      if (r.selectorText && ruleKept(r.selectorText)) continue;   // true-colour preview rule
      if (r.style) convertDecl(r.style, null);
      if (r.cssRules) walkRules(r.cssRules);
    }
  }
  // ---- true-colour areas declared by the page (meta lb-theme-keep) ----
  var keepSel = '[data-lb-theme-skip]', keepRe = null;
  function readKeep() {
    var m = document.querySelector('meta[name="lb-theme-keep"]');
    var list = m ? (m.getAttribute('content') || '').split(',').map(function (x) { return x.trim(); }).filter(Boolean) : [];
    list = list.filter(function (x) { try { document.querySelector(x); return true; } catch (e) { return false; } });
    keepSel = ['[data-lb-theme-skip]'].concat(list).join(',');
    keepRe = list.length ? new RegExp('(' + list.map(function (x) {
      return x.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    }).join('|') + ')(?![\\w-])') : null;
  }
  function ruleKept(sel) {
    if (!keepRe || !sel) return false;
    return sel.split(',').every(function (part) { return keepRe.test(part); });
  }
  function skipped(node) { return node && node.closest && node.closest(keepSel); }
  ...
  // True-colour areas often use the page's colour variables (var(--paper)),
  // which were flipped on :root — give those areas the original values back.
  var keepVarStyle = null;
  function keepVars() {
    var m = document.querySelector('meta[name="lb-theme-keep"]');
    if (!m) return;
    var seen = {}, decl = [];
    sheetRecords.forEach(function (r) {
      if (r.prop.slice(0, 2) === '--' && !seen[r.prop]) { seen[r.prop] = 1; decl.push(r.prop + ':' + r.orig); }
    });
    if (!decl.length) return;
    keepVarStyle = document.createElement('style');
    keepVarStyle.setAttribute('data-lb-theme-skip', '');
    keepVarStyle.textContent = m.getAttribute('content') + '{' + decl.join(';') + '}';
    document.head.appendChild(keepVarStyle);
  }

  function darkOff() {
    active = false;
    if (keepVarStyle) { keepVarStyle.remove(); keepVarStyle = null; }
