/* Lalabella Supabase browser client.
 * Uses only the public publishable key.
 * Supabase Auth + RLS are the security boundary; this file never uses a secret key.
 */
(function () {
  'use strict';
  ...
  if (!CONFIG || !CONFIG.url || !CONFIG.publishableKey) {
    console.error('[Lalabella] Supabase configuration is missing.');
    return;
  }
  ...
  function loadSdk() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.dataset.lalabellaSupabaseSdk = 'true';
      script.onload = () => {
        if (!window.supabase) reject(new Error('Supabase SDK is unavailable'));
  ...
  window.LalabellaSupabase = Object.freeze({
    ready() {
      if (!clientPromise) {
        clientPromise = loadSdk().then((sdk) => sdk.createClient(CONFIG.url, CONFIG.publishableKey, {
          auth: {
            persistSession: true,
